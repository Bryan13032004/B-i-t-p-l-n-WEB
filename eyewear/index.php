...
function validatePrescription($conn, $customer_id) {
    ...
    $sql = "SELECT right_sph, left_sph, pd FROM prescriptions WHERE customer_id = ? ORDER BY prescription_id DESC LIMIT 1";
    $stmt = $conn->prepare($sql);
    if ($stmt === false) {
        // In ra lỗi thực tế từ MySQL để bạn biết mình sai ở đâu (ví dụ: Table doesn't exist)
        die("<p style='color:red;'>Lỗi cú pháp SQL: " . $conn->error . "</p>");
    }
